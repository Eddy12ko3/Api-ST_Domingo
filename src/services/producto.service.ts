...
class ProductService{
    ...
    GetProducts = async () =>{
        try{
            const responseProducts = await AppDataSource.getRepository(ProductoDB)
                .find();
    ...
    GetProduct = async (id: string) =>{
            try{
            const responseProduct =  await AppDataSource.getRepository(ProductoDB)
                .findOne({where: {productoId: parseInt(id)}})
    ...
    UpdateProduct = async (id: string,{
        ...
        try{
            const productObj =  await AppDataSource.getRepository(ProductoDB)
                .findOne({where: {productoId: parseInt(id)}})
    ...
    DeleteProduct = async (id: string) =>{
        try{
            const productDelete = await AppDataSource.getRepository(ProductoDB)
                .findOne({where: {productoId: parseInt(id)}})
    
            if(!productDelete) throw new Error("PRODUCT_NOT_FOUND")
            const responseDelete = await AppDataSource.getRepository(ProductoDB).remove(productDelete)
